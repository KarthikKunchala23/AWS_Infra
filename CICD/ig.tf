resource "aws_internet_gateway" "svc_igw" {
  vpc_id = aws_vpc.ecs_vpc.id
}

resource "aws_route_table" "svc_public_rt" {
  vpc_id = aws_vpc.ecs_vpc.id

  route {
    cidr_block = "0.0.0.0/0"
    gateway_id = aws_internet_gateway.svc_igw.id
  }
}
...
resource "aws_route_table_association" "example_subnet_1_rt_assoc" {
  ...
  route_table_id = aws_route_table.svc_public_rt.id
}
